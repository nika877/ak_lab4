...
from typing import ClassVar, Generic, Literal, TypeVar, assert_never, cast, override
...
class BaseLanguageType(ABC):
    ...
    def occurs_in(
        self, target: LanguageType | InferableLanguageType, subm: SubstitutionMap
    ) -> bool:
    ...
    def _occurs_in_recursive(self, target_d, subm) -> bool:
        ...
        if isinstance(target_d, FunctionLanguageType):
            return any(
                self_d._occurs_in_recursive(a, subm) for a in target_d.arg_types
            ) or self_d._occurs_in_recursive(target_d.return_type, subm)

        return False


LanguageTypeT = TypeVar(
    "LanguageTypeT", bound="LanguageType | InferableLanguageType", covariant=True
)


PrimitiveLanguageTypeKind = Literal["VOID", "INTEGER", "FLOAT", "DOUBLE", "BOOLEAN", "STRING"]
...
class PrimitiveLanguageType(BaseLanguageType):
    ...
    DOUBLE: ClassVar[PrimitiveLanguageType]
...
class FunctionLanguageType(BaseLanguageType, Generic[LanguageTypeT]):
    ...
    def is_same(self, other: LanguageType | InferableLanguageType, subm: SubstitutionMap):
        ...
        if isinstance(other_d, FunctionLanguageType):
            return (
                ...
                and len(self.arg_types) == len(other_d.arg_types)
                and all(
                    a.is_same(b, subm)
                    for a, b in zip(self.arg_types, other_d.arg_types, strict=True)
                )
            )
        return False
    ...
    @override
    def __str__(self) -> str:
        return f"FUNC[{' '.join(str(a) for a in self.arg_types)} -> {self.return_type}]"
    ...
    def unify(
        ...
        if isinstance(other_d, FunctionLanguageType):
            ...
            if isinstance(
                new_return_type := self.return_type.unify(other_d.return_type, subm),
                UnificationError,
            ):
            ...
            return UnificationPass(
                FunctionLanguageType[InferableLanguageType](
                    cast("Sequence[InferableLanguageType]", new_arg_types),
                    cast("InferableLanguageType", new_return_type.lang_type),
                ),
                was_unified,
            )
        elif isinstance(other_d, LanguageTypeVar):
    ...
    def is_complete(self, subm: SubstitutionMap):
        return all(
            arg.is_complete(subm) for arg in self.arg_types
...
type LanguageType = PrimitiveLanguageType | FunctionLanguageType["LanguageType"]
