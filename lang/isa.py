"""ISA (Instruction Set Architecture) для процессора Nika.

Согласно варианту:
- архитектура: **acc** (аккумуляторная — единственный программно-доступный регистр acc);
- память: **neum** (фон-нейманова — единое адресное пространство);
- кодировка: **binary** (бинарное представление инструкций);
- ввод-вывод: **mem** (отображённый на память) + **trap** (прерывания);
- строки: **pstr** (Pascal-строки: cell с длиной + N cells с символами).
