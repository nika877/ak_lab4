#!/usr/bin/python3
"""Модель процессора Nika: ControlUnit + DataPath.

Вариант: **lisp | acc | neum | hw | tick | binary | trap | mem | pstr | prob1 | superscalar**.

Декодировка варианта:
- **acc** — аккумуляторная архитектура (единственный регистр данных `acc`);
- **neum** — фон-Нейманова память (код и данные в одном адресном пространстве);
- **hw** — hardwired ControlUnit (без микропрограммного ПЗУ; декодер -- комбинационная логика);
- **tick** — потактовая симуляция (одна микрооперация = один такт);
- **binary** — бинарное представление инструкций (см. `lang/isa.py`);
- **trap** — ввод/вывод через прерывания (вектор `INT_VECTOR_INPUT`, IRET);
- **mem** — порты `PORT_IN` / `PORT_OUT` отображены в память;
- **pstr** — Pascal-строки (одна ячейка с длиной + N ячеек символов);
- **prob1** — целевая программа: Project Euler #1 (сумма кратных 3 или 5 ниже N);
...
# ────────────────────────────────────────────────────────────────────────────────
#
# ASCII-схема дорожек DataPath (всё, что справа от ControlUnit):
#
# ```text
#                                  ┌─────────────────────────┐
#                                  │       memory[*]         │
#  signal_latch_addr(sel) ───────> │     (von Neumann:       │
#  ┌──────┐  PC / IR_ARG / IR_IMM  │   код + данные + порты) │
#  │ MUX  │ ─────────────────────> │                         │
#  └──────┘                        │  addr (read/write port) │
#     ^                            └─────────────────────────┘
#     │                                     │            ^
#     │ from ControlUnit                    │ data_out   │ data_in
#     │ (PC, args)                          ˇ            │
#     │                              ┌────────────┐      │
#     │                              │  data reg  │      │
#     │                              └────────────┘      │
...
#     │  ┌──────────┐  ALU op (sel)   ┌────────────┐     │
#     │  │   ALU    │ <───────────────│  arg_mux   │     │
#     │  │ (+,-,*,  │  immediate from │ IMM/MEM/   │     │
#     │  │ /,&,|,..)│  IR or mem out  │ IND_MEM    │     │
#     │  └──────────┘                 └────────────┘     │
#     │       │                                          │
#     │       │ alu_out                                  │
#     │       ˇ                                          │
...
#     │       │                                          │
#     │       ˇ                                          │
#     │  ┌─────────┐  zero, neg ──> к ControlUnit        │
#     │  │   acc   │ ─────────────> output (PORT_OUT)    │
#     │  └─────────┘                                     │
#     │       │                                          │
#     │       │ signal_wr (data = acc, addr = addr)      │
#     │       └──────────────────────────────────────────┘
#     │
#  input ────────────────> PORT_IN (через trap-механизм ControlUnit)
# ```
#
# Память -- одно-портовая (за такт можно либо читать, либо писать).
# ────────────────────────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────────────────────────
#
# ASCII-схема ControlUnit (hardwired, без микропрограммного ПЗУ):
#
# ```text
#                                       ┌──── trap_queue (input timeline) ────┐
...
#  │                                   │
#  │  ┌─────┐    ┌──────────┐          │
#  │  │ MUX │───>│ program  │──── PC ──+──────► DataPath.signal_latch_addr
#  │  │     │    │ counter  │                          │
#  +─>│     │    └──────────┘                         ˇ
#     └─────┘          ^                       ┌───────────────┐
#       ^ sel_pc       │                       │  memory[PC]   │
...
#       │              │                            ˇ
#       │              │           ┌────────────────────────────────┐
#       │              │           │  hardwired decoder + step ctr  │
#       │              │           │  (комбинационная логика по step│
#       │              │           │   и опкоду → набор сигналов)   │
#       │              │           └────────────────┬───────────────┘
#       │              │                            │ control signals
#       │              │                            ˇ
#       │              │                       DataPath (alu, latch_acc, wr…)
#       │              │                            │ zero, neg flags
#       │              └────────────────────────────┘
#       │
#       └── для JMP/JMP_T: arg -> PC (вместо +WORD_LEN)
# ```
#
# Шаги (`step`) внутри одной инструкции:
#   step 0: FETCH      — adr ← PC, oe → IR_op
#   step 1: OPERAND    — adr ← PC+WORD_LEN, oe → IR_arg (только для 2-словных)
#   step 2: ADDRESS    — для *_MEM: adr ← IR_arg, oe → data_reg;
#                        для *_IND_MEM: adr ← IR_arg, oe → data_reg (фаза 1),
#                                       adr ← data_reg, oe → data_reg (фаза 2);
#                        для *_IMM/STORE/JMP/HALT: NOP, прокидывает аргумент дальше.
#   step 3: EXECUTE    — ALU + защёлкивание acc / signal_wr / переход PC
# ────────────────────────────────────────────────────────────────────────────────
